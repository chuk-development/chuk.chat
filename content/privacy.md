...
- **Text Generation (LLMs):** Messages are sent either to OpenRouter, which routes them to open-weight AI model providers only, or directly to one of these inference providers: Fireworks AI, RunAnywhere or DeepInfra. Through OpenRouter, other inference providers can also process a request. **We only support open-weight models** (e.g., Llama, Mistral, Qwen, DeepSeek). Closed/proprietary models (Claude, GPT-4, Gemini) are not available. **Model training is disabled** - your data is not used to train AI models.
...
- **Direct providers:** Fireworks AI, RunAnywhere and DeepInfra each state in their own policy that they process prompts and responses only in memory for the request and do not store them or use them for training. They keep only metadata such as token counts for billing. We use them only for open-weight models, which these policies cover.
...
Your data may be stored and processed in Supabase infrastructure located in different countries. Several of the services listed in section 11 (for example OpenRouter, DeepInfra, Fireworks AI, Replicate, Groq, Firecrawl and PostHog) are based in the United States. Transfers to these providers are based on the EU-U.S. Data Privacy Framework where the provider is certified, and otherwise on the EU Commission's standard contractual clauses (Art. 46(2)(c) GDPR).
...
- **DeepInfra:** AI inference and embeddings, directly and via OpenRouter. [Privacy Policy](https://deepinfra.com/privacy) · [Zero Data Retention](https://docs.deepinfra.com/account/data-privacy)
